    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        if m.reply_to_message is not None and m.reply_to_message.document:
            if m.reply_to_message.document.file_name != "YouTube_PlayList.json":
                k=await m.reply("Invalid PlayList file given. Use @Mp3DownloaderRobot  or search for a playlist in @LyciBot to get a playlist file.")
                await delete_messages([m, k])
                return
            ...
            k=await m.reply("No playList file given. Use @Mp3DownloaderRobot  or search for a playlist in @LyciBot to get a playlist file.")
            await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                InlineKeyboardButton('⚡️Make Own Bot', url='https://github.com/Hi-Tec-Robot/Galaxy-Music'),
                InlineKeyboardButton('🧩 Join Here', url='https://t.me/GalaxyLanka'),
            ]
            ]
        await m.reply("<b>You can't use this bot in this group, for that you have to make your own bot from the [SOURCE CODE](https://github.com/Hi-Tec-Robot/Galaxy-Music) below.</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
